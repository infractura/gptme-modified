...
# Use a mutable container for the counter
class AutoConfirm:
    def __init__(self):
        self.count = 0
...
def parse_auto_confirm(msg: str) -> tuple[str, int]:
    """Parse :yX syntax and return cleaned message and confirmation count"""
    if msg.startswith(":y"):
        try:
            count = int(msg[2:].split()[0])
            return msg[msg.index(" "):].strip(), count
        except (ValueError, IndexError):
            return msg, 0
    return msg, 0

def smart_confirm(msg: str, no_confirm: bool = False) -> bool:
    """Confirmation function that handles auto-confirmation"""
    if auto_confirm.count > 0:
        auto_confirm.decrease()
        logger.info(f"Auto-confirming ({auto_confirm.count} remaining)")
        return True
    if no_confirm:
        return True
...
def chat(
    ...
    def confirm_func(msg) -> bool:
        return smart_confirm(msg, no_confirm)
    ...
    while True:
        ...
        if prompt_msgs:
            while prompt_msgs:
                ...
                # Check for auto-confirm syntax
                cleaned_content, count = parse_auto_confirm(msg.content)
                if count > 0:
                    auto_confirm.set(count)
                    msg = msg.replace(content=cleaned_content)
...
def prompt_user(value=None) -> str:  # pragma: no cover
    ...
    if response:
        ...
        # Check for auto-confirm syntax
        cleaned_response, count = parse_auto_confirm(response)
        if count > 0:
            auto_confirm.set(count)
            response = cleaned_response
        add_history(original_response)  # readline history
    return response
